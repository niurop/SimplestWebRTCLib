...
exports.handler = async function (event) {
  ...
  switch (event.httpMethod) {
    ...
    default:
      return response(
        '400 Bad Request: Use [GET | POST | DELETE] .../offer',
        400
      );
  }
};
  ...
  try {
    return response(
      await fauna_client.query(
        q.Select(
          ['data', 'offer'],
          q.Get(q.Match(q.Index('match'), [id, false]))
        )
      )
    );
  } catch (error) {
...
async function POST({ id }, { offer }) {
  try {
    await fauna_client.query(
      q.Create(q.Collection('webRTC-matches'), {
        data: {
          id,
          offer,
          answer: false,
...
async function DELETE({ id }) {
  try {
    await fauna_client.query(
      q.Delete(q.Select('ref', q.Get(q.Match(q.Index('match'), [id, false]))))
    );
    return response(true);
  } catch (error) {
    return response(false);
